                    ...
                    <div className="sm:flex md:items-center md:space-x-1 hidden">
                        <nav className="md:flex hidden md:space-x-4 md:items-center">
                            {[dictionary.home,dictionary.about,dictionary.projects,dictionary.contact].map((link) => (
                                <CustomLink key={link} href={`#${link.toLowerCase()}`} className="border-b-4 capitalize transition-colors border-transparent hover:border-b-main px-3 font-[400] dark:text-white text-black">{link}</CustomLink>
                            ))}
                        </nav>
                        <BaseBtn>
